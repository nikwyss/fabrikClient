<template>
<div class="doc_content " v-if="oauth.payload">
  <h1>Empfang</h1>
  <div>
      <div>
      ...
      <q-btn color="primary" :disabled="!profile.original_email" 
        v-if="!!this.profile.original_email"
        outline :label="$t('app.btn_skip')" @click="skipProfile"/>
...
export default {
  ...
  methods: {
    ...
    skipProfile: function() {
      const route = this.destination_route ? this.destination_route : {name: 'home'}
    ...
    saveProfile: function() {
      ...
      api.authProfile(this.profile).then(response => {
        ...
        // Notification
        const route = this.destination_route ? this.destination_route : {name: 'home'}
        LayoutEventBus.$emit('AfterProfileUpdate', {userEmail: this.profile.email, destination_route: route} )
