  ...
  >
    ...
    >
      <div v-if="model['id']">
        ...
        />Your are allowed to modify this ContentTree. Click on "Modify".
      </div>
      <div v-if="!model['id']">
        <q-icon
          ...
          color="white"
          style="font-size: 1.3rem;"
        />Your are allowed to add an additional ContentTree. Click on "Add".
      </div>
    ...
    >
      ...
      <h3>{{ localmodel['id'] ? 'ContentTree bearbeiten' : 'Fügen Sie einen neuen ContentTree hinzu' }} </h3>
...
export default {
  ...
  computed: {
    contenttree_count: function () {
      return this.assembly_contenttrees
        ? Object.keys(this.assembly_contenttrees).length + 1
        : 0;
    },
  ...
  methods: {
    ...
    save: function (localmodel) {
      ...
      ApiService.put(url, { contenttree: localmodel }).then((response) => {
    ...
    confirm_delete: function () {
      this.$q
        .dialog({
          ...
          message:
            "Would you like to delete this contenttree? Only Administrators can restore the data.",
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
